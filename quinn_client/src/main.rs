...
use quinn_client::stats::{OpenStreamStats, Stats};
use quinn_client::{bind_socket, noprotection::NoProtectionClientConfig};
...
struct Opt {
    /// Host to connect to
    #[clap(default_value = "localhost:9900")]
    host: String,
...
async fn run(opt: Opt) -> Result<()> {
    ...
    let mut crypto = rustls::ClientConfig::builder()
        .with_cipher_suites(quinn_client::PERF_CIPHER_SUITES)
        .with_safe_default_kx_groups()
        .with_protocol_versions(&[&rustls::version::TLS13])
        .unwrap()
        .with_custom_certificate_verifier(SkipServerVerification::new())
        .with_no_client_auth();
    crypto.alpn_protocols = vec![b"quic-server".to_vec()];
